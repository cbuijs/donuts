#!/usr/bin/env python2
# coding: utf-8
# Based on: https://raw.githubusercontent.com/hubdotcom/marlon-tools/master/tools/dnsproxy/dnsproxy.py
...
def parse_dns_message(data):
    ...
    c_id, c_fl, c_qd, c_an, c_ns, c_ar = struct.unpack('!6H', message.read(12))

    #r_code = (c_fl & 0x000f) >> 0
    #ad_flag = (c_fl & 0x0020) >> 6
    #ra_flag = (c_fl & 0x0080) >> 7
    #rd_flag = (c_fl & 0x0100) >> 8
    #tc_flag = (c_fl & 0x0200) >> 9
    #aa_flag = (c_fl & 0x0400) >> 10
    #op_code = (c_fl & 0x7800) >> 11
    #qr_flag = (c_fl & 0x8000) >> 15

    print('RCODE: {0}'.format(r_code))
    print('ID: {0}'.format(c_id))
    print('OPCODE: {0}'.format(op_code))
    # parse question
...
class DNSProxyHandler(BaseRequestHandler):
    def handle(self):
        ...
        q = req.question
        if q.type_ in dnsrrtype and q.class_ in dnsclass:
            ...
            print('LOOKUP: {0} {1} {2}'.format(q.name, dnsrrtype[q.type_], dnsclass[q.class_]))
